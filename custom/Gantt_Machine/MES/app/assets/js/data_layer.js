    ...
    try {
        ...
        let consumables_data = await fetch_consumables()
           ...
           let gantt_timeline_data = []
            ...
            console.log(gantt_timeline_data,"<<<<<<<gantt_timeline_data>>>>>>>")
...
console.log(gantt_consumables_data.concat(gantt_timeline_data),"<<<<<<<<<<<<gantt_consumables_data.concat(gantt_timeline_data)")
        gantt.parse({
            data:gantt_consumables_data.concat(gantt_timeline_data)
        });
        window.app_consumer.fetch_functions()
        lightboxConfig()
...
async function gantt_parse_timeline(timeline,label="") {
    console.log(timeline,label,"<<<<<<<<otherlabel")
    ...
    $.each(timeline.data, (index, slot_data) => {
        let start_date = moment(slot_data[window.app_consumer.t_start_link_name], window.app_consumer.creator_date_format),
            ...
            console.log(slot_data,"<<<<<<<<slotData")
...
async function fetch_timeline() {
    ...
    try {
        ...
        try {
            ...
            window.app_consumer[`${t_report_name}`] = t_report_name?.data
...
async function fetch_consumables() {
    ...
    try {
        ...
        try {
            ...
            console.log(consumables_data,"<<<<<<<<<<<<<ssss")
            window.app_consumer[`${window.app_consumer.c_report_name}`] = consumables_data?.data
